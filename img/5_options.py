...
def fMergeImages():
    try:
        #options
        img_width=cbPicWidth.get()
        if img_width=='Origin':
            img_width=-1
        else:
            img_width=int(img_width)
        img_space=cbPicGap.get()
        if img_space=='Narrow':
            img_space=30
            ...
            img_space=0
        img_format=cbPicFormat.get().lower()

        images=[Image.open(img) for img in listFile.get(0,END)]
        
        image_sizes=[]
        if img_width>-1:
            image_sizes=[(int(img_width),int(img_width*img.size[1]/img.size[0])) for img in images]
        else:
        ...
        imgResult=Image.new('RGB',(max(widths),sum(heights)+(img_space*(len(images)-1))),(255,255,255))
        offsetY=0
        for idx, img in enumerate(images):
            if img_width>-1:
                img=img.resize(image_sizes[idx])
            imgResult.paste(img,(0,offsetY))
            offsetY+=img.size[1]+img_space
            statusPB.set((idx+1)/len(images)*100)
            # statusPB = 100
            pbMerging.update()
        file_name='MergedPicture.'+img_format
        imgResult.save(os.path.join(pathSaving.get(),file_name))
